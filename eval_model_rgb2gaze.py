...
from egom2p.utils.plotting_utils import decode_gaze
...
ckpt = torch.load('./ckpt/checkpoint-gaze.pth', map_location='cpu', weights_only=False)
gaze_tok = get_tok_model(ckpt['args'], device)
gaze_tok.load_state_dict(ckpt['model'])
gaze_tok.eval()
...
decoding_steps = [5] * len(target_domains)
...
for file in glob.glob('example_data/rgb2gaze_*.npz'): # make sure all videos are in 8fps, 256x256, 2-second
    ...
    dec_dict = decode_gaze(
        name,
        out_dict, toks,
        image_size=256, patch_size=8,
        name = 'example_data/rgb2gaze'
    )

    print(f'predicting {file}')

print(f'use vis_3d/vis_gaze.py to visualize predicted gaze dynamics!')