...
class CausalVideoTokenizer(torch.nn.Module):
    ...
    def forward(
        ...
        for idx in range(0, (num_frames - 1) // temporal_window + 1):
            ...
            output_token = self.autoencode(input_tensor)
            ...
            # print(output_token.shape) # [1, 5, 60, 60]
            output_token_list.append(output_token.detach().cpu().numpy())
        return np.concatenate(output_token_list, axis=1)
