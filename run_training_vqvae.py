...
from egom2p.data.cam_traj_dataset import CamTrajDataset
from egom2p.data.gaze_dataset import GazeDataset
from egom2p.data.modality_info import MODALITY_INFO, MODALITY_TRANSFORMS_DIVAE
from egom2p.utils import ModelEmaV2 as ModelEma
from egom2p.utils import NativeScalerWithGradNormCount as NativeScaler
from egom2p.utils.data_constants import CAM_STD, CAM_MEAN
from egom2p.utils.optim_factory import create_optimizer
from egom2p.vq.vq_utils import compute_codebook_usage, rot6d_to_rotmat, rotation_distance
from egom2p.vq.vqvae import VQVAE
from egom2p.data.functional import denormalize, denormalize_cam
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--num_frames', default=32, type=int,
                        help='video clip frame number')
    parser.add_argument('--patch_size', default=16, type=int,
                        help='Patch size for ViT encoder (default: %(default)s)')
    parser.add_argument('--tubelet_size', default=[2, 16, 16], type=list,
                        help='tubelet size for ViViT encoder, tubelet_size[1] == tubelet_size[2] == patch_size')
    parser.add_argument('--temporal_downsampling', default=None, type=int, help='temporal downsampling for vqvae')
    ...
    parser.add_argument('--encoder_type', default='vivit_b_enc', type=str, metavar='ENC',
                        help='Name of encoder (default: %(default)s)')
    parser.add_argument('--decoder_type', default='vivit_b_dec', type=str, metavar='DEC',
    ...
    parser.add_argument('--no_init_weights', action='store_false', dest='init_weights')
    parser.set_defaults(init_weights=True)
    ...
    parser.add_argument('--reconst_weight', default=1.0, type=float,
                        help='Weight of reconstruction loss (default: %(default)s)')
    ...
    # Dataset parameters
    parser.add_argument('--prefix', default='/store/swissai/a03/datasets/holoassist', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--crop_size', default=256, type=int, help='center crop during training')
    ...
    parser.add_argument('--num_aug_sample', default=0, type=int, help="Repeated_aug (default: 1)") # set 0, not do augmentation
    parser.add_argument('--train_interpolation', type=str, default='bicubic',
                        help='Training interpolation (random, bilinear, bicubic default: "bicubic")')
    parser.add_argument('--aa', type=str, default='rand-m5-n2-mstd0.25-inc1', metavar='NAME',
    ...
    parser.add_argument('--use_npy', action='store_true', help='load npy or load mp4')
    parser.add_argument('--no_use_npy', action='store_false', dest='use_npy')
    parser.set_defaults(use_npy=False)
    ...
    parser.add_argument('--tokenize_path', default='', type=str)
    parser.add_argument('--tokenize_save_path', default='', type=str)
...
def get_model(args: argparse.Namespace, device: Union[torch.device, str]) -> VQVAE:
    ...
    model = VQVAE(
        ...
        init_encoder=args.init_weights,
        temporal_compress=args.temporal_downsampling,
    )
...
def get_perceptual_loss(args: argparse.Namespace, device: Union[torch.device, str]) -> Optional[nn.Module]:
    ...
    if args.percept_loss_type == 'lpips':
        from egom2p.vq.percept_losses.lpips import LPIPS
        percept_loss = LPIPS().to(device)
        percept_loss.percept_transform = lambda x: x # No transform needed for LPIPS. Expects [-1, 1] range
    else:
        from egom2p.vq.percept_losses.timm_perceptual_loss import TimmPerceptualLoss
...
def main(args: argparse.Namespace) -> None:
    ...
    # TODO: change image augmentation to video augmentation. maybe https://github.com/facebookresearch/AugLy/tree/main/augly/video
    ...
    # dataset_train = VideoDataset(prefix='/nfs/tang.scratch.inf.ethz.ch/export/tang/cluster/genli/holoassist', args=args)
    if args.tokenize:
        if args.domain in ['cam']:
            dataset_tokenize = CamTrajDataset(mode='tokenize', args=args, clip_len=args.num_frames)
        elif args.domain in ['gaze']:
            dataset_tokenize = GazeDataset(mode='tokenize', args=args, clip_len=args.num_frames)
        elif args.domain in ['rgb']:
            dataset_tokenize = VideoDataset(mode='tokenize', args=args, clip_len=args.num_frames)
            
        sampler_tokenize = torch.utils.data.SequentialSampler(dataset_tokenize)
        data_loader_tokenize = torch.utils.data.DataLoader(
                                dataset_tokenize, sampler=sampler_tokenize,
        ...
        if args.domain in ['rgb', 'depth']:
            dataset_train = VideoDataset(args=args, clip_len=args.num_frames)
        elif args.domain in ['cam']:
            dataset_train = CamTrajDataset(args=args, clip_len=args.num_frames)
        elif args.domain in ['gaze']:
            dataset_train = GazeDataset(args=args, clip_len=args.num_frames)
        else:
            pdb.set_trace()
        ...
        if args.eval_data_path:
            ...
            # dataset_val = VideoDataset(prefix='/nfs/tang.scratch.inf.ethz.ch/export/tang/cluster/genli/holoassist', mode='val', args=args)
            if args.domain in ['rgb', 'depth']:
                dataset_val = VideoDataset(mode='val', args=args, clip_len=args.num_frames)
            elif args.domain in ['cam']:
                dataset_val = CamTrajDataset(mode='val', args=args, clip_len=args.num_frames)
            elif args.domain in ['gaze']:
                dataset_val = GazeDataset(mode='val', args=args, clip_len=args.num_frames)
                dataset_val_plot = GazeDataset(mode='val_plot', args=args, clip_len=args.num_frames)
            else:
                pdb.set_trace()
            ...
            if args.dist_eval:
                ...
                if args.domain == 'gaze':
                    sampler_val_plot = torch.utils.data.DistributedSampler(
                    dataset_val_plot, num_replicas=num_tasks, rank=global_rank, shuffle=False)
            else:
                sampler_val = torch.utils.data.SequentialSampler(dataset_val)
                if args.domain == 'gaze':
                    sampler_val_plot = torch.utils.data.SequentialSampler(dataset_val_plot)
            ...
            if args.domain == 'gaze':
                data_loader_val_plot = torch.utils.data.DataLoader(
                    dataset_val_plot, sampler=sampler_val_plot,
                    batch_size=int(1.5 * args.batch_size) if args.batch_size_eval is None else args.batch_size_eval,
                    num_workers=args.num_workers,
                    pin_memory=args.pin_mem,
                    drop_last=False,
            ...
            data_loader_image_log = data_loader_val if args.domain != 'gaze' else data_loader_val_plot
    ...
    # TODO!
    # args.lr = args.blr * total_batch_size * args.num_frames / 256
    args.lr = args.blr * total_batch_size / 256
    total_batch_img_size = total_batch_size * args.num_frames
    print("LR = %.8f" % args.lr)
    print("Batch size = %d" % total_batch_size)
    print("Batch image size = %d" % total_batch_img_size)
    ...
    if args.eval_only:
        ...
        for eval_img_size in eval_image_sizes:
            eval_metrics_results = eval_metrics(
                ...
                log_writer=log_writer, log_codebook_usage=args.log_codebook_usage, dataset_train=dataset_train
            )
            if log_writer is not None:
            ...
            eval_image_log(
                ...
                dtype=dtype, num_logged_images=args.num_logged_images, mask_value=args.mask_value, log_writer=log_writer, dataset_train=dataset_train
            )

        exit(0)
    
    if args.tokenize:
        save_path = os.path.join(args.tokenize_save_path, f'token')
        os.makedirs(save_path, exist_ok=True)
        tokenize(model, data_loader_tokenize, device, save_path, dtype)
        exit(0)
...
def compute_reconst_loss(
    ...
    if loss_fn == 'mse':
    ...
    elif loss_fn == 'mse_mask':
        # reconst_loss = F.mse_loss(model_output, images[..., :2] * images[..., 2:])
        reconst_loss = (((model_output - images[..., :2]) ** 2) * images[..., 2:]).sum() / images[..., 2:].sum()
    elif loss_fn == 'l1':
        ...
        reconst_loss = F.cross_entropy(model_output, images)
    elif loss_fn == 'geodesic':
        # BxTxNx3. N[0,1]: rot6d. N[2]: transl
        B, T, _, _ = images.shape
        dtype = images.dtype
        device = images.device

        gt_pose = torch.eye(4, dtype=dtype, device=device).view(1, 1, 4, 4)
        gt_pose = gt_pose.expand(B, T, -1, -1).clone()
        gt_pose[:, :, :3, :3] = rot6d_to_rotmat(images[:, :, :2].flatten(2))
        gt_pose[:, :, :3, 3] = images[:, :, 2]

        recon_pose = torch.eye(4, dtype=dtype, device=device).view(1, 1, 4, 4)
        recon_pose = recon_pose.expand(B, T, -1, -1).clone()
        recon_pose[:, :, :3, :3] = rot6d_to_rotmat(model_output[:, :, :2].flatten(2))
        recon_pose[:, :, :3, 3] = model_output[:, :, 2]
...
def train_one_epoch(model: Union[nn.Module, DDP], 
                    ...
                    dataset_train = None) -> Dict[str, float]:
    ...
    for step, x in enumerate(metric_logger.log_every(data_loader, print_freq, iter_len=loader_len, header=header)):
        ...
        if step_eval:
            ...
            eval_stats = launch_evals(
                ...
                log_writer=log_writer, log_codebook_usage=log_codebook_usage, dataset_train=dataset_train
                )
        ...
        images = x.to(device, non_blocking=True)
        ...
        with torch.cuda.amp.autocast(dtype=dtype_percept, enabled=dtype_percept != torch.float32):
            if percept_loss_fn is not None and percept_loss_weight > 0:
                ...
                percept_loss = percept_loss_fn(rearrange(model_output, 'b c n h w -> (b n) c h w').to(dtype_percept), \
                                               rearrange(images, 'b c n h w -> (b n) c h w').to(dtype_percept)).mean()
            else:
                percept_loss = torch.tensor(0.0)

        loss = args.reconst_weight * reconst_loss + codebook_weight * code_loss + percept_loss_weight * percept_loss
...
def eval_metrics(model: Union[nn.Module, DDP], 
    ...
    if domain in ['rgb', 'normal']:
        ...
        fid_metric, lpips_metric, inception_metric, mae_metric_rot = None, None, None, None
    ...
    for x in metric_logger.log_every(data_loader, print_freq=10, header=f'{prefix} Image metrics @{eval_size}:'):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=dtype, enabled=dtype != torch.float32):
            ...
            output = unwrap_model(model).decode_quant(quant).float() # TODO: might be nan!
        ...
        if domain in ['rgb', 'normal']:
            ...
            gt = denormalize_cam(images, mean=mean, std=std)
            reconst = denormalize_cam(output, mean=mean, std=std)
        elif domain in ['gaze']:
            gt = denormalize_cam(images[..., :2], mean=(0.5, 0.5), std=(0.5, 0.5))
            reconst = denormalize_cam(output, mean=(0.5, 0.5), std=(0.5, 0.5))
        else:
        ...
        # Compute metrics
        if domain in ['rgb']:
            # TODO: naive implementation to calc metrics on frames
            for idx in range(gt.shape[2]):
                f_recon = reconst[:, :, idx].contiguous()
                f_gt = gt[:, :, idx].contiguous()
                mse_metric.update(f_recon, f_gt)
                mae_metric.update(f_recon, f_gt)
                psnr_metric.update(f_recon, f_gt)
                if ms_ssim_metric is not None:
                    ms_ssim_metric.update(f_recon, f_gt)
                if fid_metric is not None:
                    fid_metric.update(f_gt, real=True)
                    fid_metric.update(f_recon.clamp(0, 1), real=False)
                if lpips_metric is not None:
                    lpips_metric.update(f_recon.clamp(0, 1), f_gt)
                if inception_metric is not None:
                    inception_metric.update(f_recon.clamp(0, 1))
                if iou_metric is not None:
                    iou_metric.update(f_recon.clamp(0, 1), f_gt)
        elif domain in ['cam']:
            gt_rotmat = rot6d_to_rotmat(gt[:, :, :6])
            recon_rotmat = rot6d_to_rotmat(reconst[:, :, :6])
            gt_transl = gt[:, :, 6:]
            recon_transl = reconst[:, :, 6:]
            mae_metric.update(recon_transl, gt_transl)

            rot_err = rotation_distance(gt_rotmat, recon_rotmat)
            mae_metric_rot = MeanAbsoluteError(sync_on_compute=True, compute_on_cpu=compute_on_cpu).to(device)
            mae_metric_rot.update(rot_err, torch.zeros(*rot_err.shape).to(rot_err.device))
        elif domain in ['gaze']:
            for bz in range(gt.shape[0]):
                mask = images[bz, :, 2]
                id = torch.where(mask)[0]
                _reconst = reconst[bz][id].contiguous()
                _gt = gt[bz][id].contiguous()
                mse_metric.update(_reconst, _gt)
                mae_metric.update(_reconst, _gt)
                psnr_metric.update(_reconst, _gt)
    ...
    if domain in ['cam']:
        results[prefix + 'MAE-transl' + suffix] = mae_metric.compute().item()
    else:
    ...
    if mae_metric_rot is not None:
        results[prefix + 'Cam Rot Degree' + suffix] = mae_metric_rot.compute().item()
...
@torch.no_grad()
def tokenize(model: Union[nn.Module, DDP], 
            data_loader: Iterable, 
            device: Union[torch.device, str],
            save_path: str,
    ...
    if utils.is_main_process():
        ...
        for x in data_loader:
            # Load inputs, move to device, optionally extract features, resize, and optionally add mask
            # images = prepare_inputs(x, domain, feature_extractor, device, image_size=eval_size, mask_value=mask_value)
            images = x['x'].to(device, non_blocking=True)
            name = x['name']
            # Autoencode the images
            if not args.recon:
                with torch.no_grad(), torch.cuda.amp.autocast(dtype=dtype, enabled=dtype != torch.float32):
                    tokens = unwrap_model(model).tokenize(images)

                for idx in range(images.shape[0]):
                    print('save ', name[idx])
                    np.savez_compressed(os.path.join(save_path, name[idx]), tokens[idx].detach().cpu().numpy())
            else:
                if 'cam' in args.resume:
                    mean = np.array(CAM_MEAN)
                    std = np.array(CAM_STD)
                elif 'gaze' in args.resume:
                    mean = (0.5, 0.5)
                    std = (0.5, 0.5)
                with torch.no_grad(), torch.cuda.amp.autocast(dtype=dtype, enabled=dtype != torch.float32):
                    recon = unwrap_model(model).autoencode(images)
                rec = denormalize_cam(recon, mean=mean, std=std)
                # return rec.detach().cpu().numpy() 
                for idx in range(images.shape[0]):
                    print('save ', name[idx])
                    np.save(os.path.join(save_path, name[idx]), rec[idx].detach().cpu().numpy())
    ...
    if utils.is_main_process():
        ...
        for x in data_loader:
            ...
            if domain in ['cam']:
                ...
                reconst = reconst.float().cpu().numpy()
                print('save to ', args.output_dir)
                for idx, b in enumerate(gt):
                    np.save(os.path.join(args.output_dir, 'gt_%d.npy' % idx), b)
                for idx, b in enumerate(reconst):
                    np.save(os.path.join(args.output_dir, 'recon_%d.npy' % idx), b)
            elif domain in ['gaze']:
                gt = gt.float().cpu().numpy()
                reconst = reconst.float().cpu().numpy()
                print('save to ', args.output_dir)
                ...
                for idx, b in enumerate(gt):
                    np.savez(os.path.join(args.output_dir, 'gt_%d' % idx), gaze=b, video_id=video_id[idx], \
                             start_frame=start_frame[idx], end_frame=end_frame[idx], \
                             mask=images[idx, :, 2].float().cpu().numpy())
                    
                for idx, b in enumerate(reconst):
                    np.savez(os.path.join(args.output_dir, 'recon_%d' % idx), gaze=b, video_id=video_id[idx], \
                             start_frame=start_frame[idx], end_frame=end_frame[idx])
            else:
                # Save some example images
                if gt.ndim == 5 and gt.shape[1] == 3:
                    # Handle 3-channel images
                    gt_bytes = (255 * gt.float().permute(0,2,3,4,1).clamp(0,1).cpu().numpy()).astype(np.uint8)
                    reconst_bytes = (255 * reconst.float().permute(0,2,3,4,1).clamp(0,1).cpu().numpy()).astype(np.uint8)
                else:
                ...
                for idx, b in enumerate(gt_bytes):
                    write_video(os.path.join(args.output_dir, 'gt_%03d.mp4' % idx), b, fps=8)
                for idx, b in enumerate(reconst_bytes):
                    write_video(os.path.join(args.output_dir, 'recon_%03d.mp4' % idx), b, fps=8)
            exit(0)
