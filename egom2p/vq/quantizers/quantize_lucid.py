...
class VectorQuantize(nn.Module):
    ...
    def indices_to_embedding(self, indices):
        ...
        if embedding.ndim == 3:
            ...
            embedding = rearrange(embedding, 'b t v c -> b c t v')
        return embedding

    def forward(self, x):
        shape, device, heads, is_multiheaded, codebook_size, dim = x.shape, x.device, self.heads, self.heads > 1, self.codebook_size, x.dim()
        need_transpose = not self.channel_last and not self.accept_image_fmap

        if self.accept_image_fmap and dim == 5:
            time, height, width = x.shape[-3:]
            x = rearrange(x, 'b c t h w -> b (t h w) c')
        
        if dim == 3:
            # b d t. for cam traj now.
            time = x.shape[-1]
        ...
        if self.accept_image_fmap and dim == 5:
            quantize = rearrange(quantize, 'b (t h w) c -> b c t h w', t = time, h = height, w = width)
            embed_ind = rearrange(embed_ind, 'b (t h w) ... -> b t h w ...', t= time, h = height, w = width)
            if is_multiheaded:
                embed_ind = rearrange(embed_ind, 'b t h w ... -> b ... t h w')

        if dim == 4:
            quantize = rearrange(quantize, 'b (t v) c -> b c t v', t = time, v=nverts)
            embed_ind = rearrange(embed_ind, 'b (t v) ... -> b t v ...', t= time, v=nverts)
            if is_multiheaded:
                pdb.set_trace()
        elif dim == 3:
            quantize = rearrange(quantize, 'b t c -> b c t', t = time)
            embed_ind = rearrange(embed_ind, 'b t ... -> b t ...', t= time)
            if is_multiheaded:
                pdb.set_trace()
