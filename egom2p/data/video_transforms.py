...
from egom2p.data.rand_augment import rand_augment_transform
...
import torchvision
import cv2
import egom2p.data.functional as FF
...
def random_scaling(video, scale_range=(1.0, 1.2)):
    t, h, w, c = video.shape
    scale = np.random.uniform(scale_range[0], scale_range[1])
    new_h, new_w = int(h * scale), int(w * scale)
    video_resized = np.array([cv2.resize(frame, (new_w, new_h), interpolation=cv2.INTER_LINEAR) for frame in video])
    
    # Crop the video to original size
    crop_h = (new_h - h) // 2
    crop_w = (new_w - w) // 2
    return video_resized[:, crop_h:crop_h+h, crop_w:crop_w+w, :]

def adjust_brightness(video, factor_range=(0.7, 1.3)):
    factor = np.random.uniform(factor_range[0], factor_range[1])
    return np.clip(video * factor, 0, 255).astype(np.uint8)

def random_rotation(video, angle_range=(-10, 10)):
    t, h, w, c = video.shape
    angle = np.random.uniform(angle_range[0], angle_range[1])
    M = cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1)
    rotated_video = np.array([cv2.warpAffine(frame, M, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT_101) for frame in video])
    return rotated_video

def horizontal_flip_np(video):
    return np.flip(video, axis=2)

def random_rotation_dep(video, angle_range=(-5, 5)):
    t, h, w = video.shape
    angle = np.random.uniform(angle_range[0], angle_range[1])
    M = cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1)
    rotated_video = np.array([cv2.warpAffine(frame, M, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REFLECT_101) for frame in video])
    return rotated_video

def add_gaussian_noise(video, mean=0, std=0.01):
    noise = np.random.normal(mean, std, video.shape)
    noisy_video = video + noise
    return np.clip(noisy_video, 0, 1)
...
class BNormalize(object):
    """
    batch version of the normalize
    """
    def __init__(self, mean, std):
        self.mean = mean
        self.std = std
    ...
    def __call__(self, clip):
        """
        Args:
            clip (Tensor): Tensor clip of size (B, C, T, H, W) to be normalized.
        Returns:
            Tensor: Normalized Tensor clip.
        """
        return FF.bnormalize(clip, self.mean, self.std)