...
from egom2p.data.modality_transforms import get_transform_key
from egom2p.utils import to_2tuple
from egom2p.utils.tokenizer import get_sentinel_to_id_mapping
...
class UnifiedMasking(object):
    def __init__(self,
        ...
        self.mod_is_not_seq = torch.tensor([mod['type'] in ['img', 'cam', 'gaze', 'keypoints'] for mod in modality_info.values()])
    ...
    def target_token_budget(self, input_token_budget, num_target_tokens, dir_idx=0):
        ...
        max_tokens_remaining = torch.where(self.mod_is_not_seq, self.max_tokens - torch.tensor(input_token_budget), self.max_tokens)
    ...
    def __call__(self, mod_dict):
        ...
        for (mod_name, mod_info), input_budget, target_budget in zip(self.modality_info.items(), input_token_budget, target_token_budget):
            ...
            if mod_type in ['img', 'cam', 'gaze', 'keypoints']: # treat all modalities as a whole. not a sequence.
