...
from egom2p.data.masking import TransferMasking, UnifiedMasking
from egom2p.data.modality_transforms import (CropSettingsTransform, IdentityTransform,
                                      MaskTransform, UnifiedDataTransform,
                                      get_transform_key)
from egom2p.data.multimodal_dataset_folder import MultiModalDatasetFolder
from egom2p.utils.dist import get_rank, get_world_size
...
def build_pretraining_dataset(
    ...
    """Builds the EgoM2P pre-training dataset based on the given arguments.
    This function should mainly used for smaller datasets (e.g. validation sets), 
    while large training sets should be loaded with build_wds_pretraining_dataloader in webdataset format.
    ...
    Returns:
        EgoM2P pre-training dataset as a PyTorch Dataset.
    """
...
def build_transfer_dataset(
    ...
    """Builds the EgoM2P transfer dataset based on the given arguments.
    ...
    Returns:
        EgoM2P transfer dataset as a PyTorch Dataset.
    """
...
def build_wds_pretraining_dataloader(
    ...
    """Builds the WebDataset EgoM2P pre-training dataloader based on the given arguments.
    ...
    Returns:
        EgoM2P pre-training dataloader as a WebDataset DataLoader.
    """
...
def make_empty_mod_dict(modality_info):
    ...
    for mod_name, mod_info in modality_info.items():
        ...
        if 'num_channels' in mod_info and 'input_size' in mod_info:
            ...
            if mod_name in ["tok_rgb", "tok_depth"]:
                empty_mod['tensor'] = torch.zeros((5, 32, 32), dtype=torch.int32)
                max_tokens = mod_info['max_tokens']
            else:
                max_tokens = mod_info['max_tokens']
                empty_mod['tensor'] = torch.zeros((max_tokens), dtype=torch.int32)
