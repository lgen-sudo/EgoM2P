...
from egom2p.data.modality_transforms import get_transform_key, get_transform_resolution, MetadataTransform
from egom2p.utils.data_constants import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, CAM_MEAN, CAM_STD
from egom2p.utils import denormalize, get_sentinel_to_id_mapping, merge_span_masking
from egom2p.utils.generation import unbatch
from egom2p.data.functional import denormalize_cam
import pdb
...
def decode_tok_vid(filename, mod_dict, tokenizers, key='tok_rgb', image_size=256, patch_size=16, name='tmp'):
    """
    Decodes a sequence of RGB tokens from a model dictionary into an RGB image.
    ...
    Args:
        ...
        verbose (bool): Whether to print the decoding progress.
    """
    if mod_dict[key]['tensor'].ndim != 4:
        img_tok = rearrange(mod_dict[key]['tensor'], "b (nt nh nw) -> b nt nh nw", nt=5, nh=32, nw=32)
    else:
        img_tok = mod_dict[key]['tensor']
        
    rec = tokenizers[get_transform_key('tok_vid')].decode(img_tok)
    padded_output_video = tensor2numpy(rec)
    output_video = unpad_video_batch(padded_output_video, [0, 0, 0, 16, 256, 256]) # refer to cosmos for reasons
    if key == 'tok_depth':
        mean = np.mean(output_video, axis=-1, keepdims=True)
        output_video[..., :] = mean

    os.makedirs(f'{name}/', exist_ok=True)
    output_path = f'{name}/{filename}_{key}.mp4'
    write_video(output_path, output_video[0], fps=8)
    print('decode ', key, ' to ', output_path)
    

def decode_gt_vid(img_tok_path, tokenizers):
    """ 
    
    !!!!!!!!!!!!!!the new decode code is above!!!!!!!!!!!!!!!!!!
    ...
    Args:
        ...
        verbose (bool): Whether to print the decoding progress.
    """
    img_tok = torch.as_tensor(np.load(img_tok_path)['arr_0']).unsqueeze(0).cuda()

    rec = tokenizers[get_transform_key('tok_vid')].decode(img_tok)
    padded_output_video = tensor2numpy(rec)
    output_video = unpad_video_batch(padded_output_video, [0, 0, 0, 16, 256, 256])

    output_path = f'./eval_model_res_3/{os.path.basename(img_tok_path)}_gt.mp4'
    write_video(output_path, output_video[0], fps=8)

def decode_tok_cam(filename, mod_dict, tokenizers, key='tok_cam', name='tmp'):
    cam_tok = mod_dict[key]['tensor']
    rec = tokenizers[key].decode_tokens(cam_tok)
    mean = np.array(CAM_MEAN)
    std = np.array(CAM_STD)
    rec = denormalize_cam(rec, mean=mean, std=std)
    # return rec.detach().cpu().numpy() 
    os.makedirs(f'{name}/', exist_ok=True)
    output_path = f'{name}/{filename}_{key}.npy'
    np.save(output_path, rec.detach().cpu().numpy()[0])
    print('decode ', key, ' to ', output_path)

def decode_tok_gaze(filename, mod_dict, tokenizers, key='tok_gaze', name='tmp'):
    gaze_tok = mod_dict[key]['tensor']
    rec = tokenizers[key].decode_tokens(gaze_tok)
    rec = denormalize_cam(rec[..., :2], mean=(0.5, 0.5), std=(0.5, 0.5))
    # return rec.detach().cpu().numpy()
    os.makedirs(f'{name}/', exist_ok=True)
    output_path = f'{name}/{filename}_{key}.npy'
    # output_path = f'/iopsstor/scratch/cscs/lgen/eval_result/egoexo_gaze/v1/{filename}_{key}.npy'
    np.save(output_path, rec.detach().cpu().numpy()[0])
    print('decode ', key, ' to ', output_path)

def decode_gt_gaze(tokenizers, gaze_tok_path='/capstor/scratch/cscs/lgen/datasets_aligned/val/gaze/holoassist/R0027-12-GoPro_10.npz'):
    # gaze_tok = mod_dict[key]['tensor']
    gaze_tok = torch.as_tensor(np.load(gaze_tok_path)['arr_0']).unsqueeze(0).cuda()
    rec = tokenizers['tok_gaze'].decode_tokens(gaze_tok)
    rec = denormalize_cam(rec[..., :2], mean=(0.5, 0.5), std=(0.5, 0.5))
    # return rec.detach().cpu().numpy()
    output_path = f'./eval_model_res_3/{os.path.basename(gaze_tok_path)}_gaze_quantized_gt.npy'
    np.save(output_path, rec.detach().cpu().numpy()[0])
    # print('decode ', key, ' to ', output_path)
    # exit(0)
...
# just a copy from decode_dict()
def decode_depth(filename, mod_dict, tokenizers, image_size=224, patch_size=16, seed=None, name=None):
    dec_dict = {}
    for key in mod_dict:
        k, res = get_transform_key(key), get_transform_resolution(key, image_size, to_tuple=False)
        if k == 'tok_rgb':
            continue # do not decode input rgb tokens
        elif k == 'tok_depth':
            decoded = decode_tok_vid(
                filename,
                mod_dict, tokenizers, key=key, name=name
            )     
        dec_dict[key] = decoded
    return dec_dict

# just a copy from decode_dict()
def decode_rgb(filename, mod_dict, tokenizers, image_size=224, patch_size=16, seed=None, name=None):
    dec_dict = {}
    for key in mod_dict:
        k, res = get_transform_key(key), get_transform_resolution(key, image_size, to_tuple=False)
        if k == 'tok_depth':
            continue # do not decode input depth tokens
        elif k == 'tok_rgb':
            decoded = decode_tok_vid(
                filename,
                mod_dict, tokenizers, key=key, name=name
...
# just a copy from decode_dict()
def decode_cam(filename, mod_dict, tokenizers, image_size=224, patch_size=16, seed=None, name=None):
    dec_dict = {}
    for key in mod_dict:
        k, res = get_transform_key(key), get_transform_resolution(key, image_size, to_tuple=False)
        if k == 'tok_rgb':
...
# just a copy from decode_dict()
def decode_gaze(filename, mod_dict, tokenizers, image_size=224, patch_size=16, seed=None, name=None):
    dec_dict = {}
    for key in mod_dict:
        k, res = get_transform_key(key), get_transform_resolution(key, image_size, to_tuple=False)
        if k == 'tok_rgb':
...
def decode_dict(filename, mod_dict, tokenizers, image_size=224, patch_size=16, 
                decoding_steps=25, activate_controlnet=False, controlnet_guidance_scale=2.5, controlnet_cond_scale=0.8,
                to_rgb=True, seed=None, name=None):
    """
    ...
    for key in mod_dict:
        ...
        if k == 'rgb':
            ...
            # decoded_gt = decode_gt_vid('/capstor/scratch/cscs/lgen/datasets_aligned/val/depth/holoassist/R0027-12-GoPro_10.npz', tokenizers)
        elif k == 'tok_semseg':
