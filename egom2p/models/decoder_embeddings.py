...
import pdb
from .egom2p_utils import build_1d_sincos_posemb, build_2d_sincos_posemb, pair, build_3d_sincos_posemb
...
class SequenceDecoderEmbedding(nn.Module):
    ...
    def init(self, dim_tokens: int = 768, init_std=0.02):
        ...
        Should be called when setting up EgoM2P.
...
class ImageTokenDecoderEmbedding(nn.Module):
    ...
    def forward_embed(self, d: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dict[str, torch.Tensor]: Modality dict with added keys:
                - 'x' (torch.Tensor): Embedded token sequence, which is replaced by mask tokens in the EgoM2P decoder. Shape (B, H*W, D) where D is the embedding dimension.
...
class GazeCamTokenDecoderEmbedding(nn.Module):
    """Embedding module for tokenized spatial inputs.
    ...
    def init(self, dim_tokens: int = 768, init_std=0.02):
        ...
        # Fixed-size positional embeddings. Can be interpolated to different input sizes
        if self.sincos_pos_emb:
            pos_emb = build_1d_sincos_posemb(30, embed_dim=self.dim_tokens)
            self.register_buffer("pos_emb", pos_emb)
        else:
            self.pos_emb = nn.Parameter(torch.zeros(1, 30, self.dim_tokens))
            nn.init.normal_(self.pos_emb, std=init_std)
    ...
    def forward_embed(self, d: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        assert x.shape[1] == x_emb.shape[1]
...
class VideoTokenDecoderEmbedding(nn.Module):
    """Embedding module for tokenized spatial inputs.
    ...
    def __init__(self,
                 ...
                 patch_size: Union[int, Tuple[int,int,int]] = (4, 8, 8),
                 dim_tokens: Optional[int] = None,
                 sincos_pos_emb: bool = True,
    ...
    def init(self, dim_tokens: int = 768, init_std=0.02):
        ...
        # Task embedding identifying from which task a given token comes from
        # Fixed-size positional embeddings. Can be interpolated to different input sizes
        h_posemb = self.image_size[0] // self.patch_size[1]
        w_posemb = self.image_size[1] // self.patch_size[2]
        if self.sincos_pos_emb:
            pos_emb = build_3d_sincos_posemb(t=5, h=h_posemb, w=w_posemb, embed_dim=self.dim_tokens)
            self.register_buffer("pos_emb", pos_emb)
        else:
            self.pos_emb = nn.Parameter(torch.zeros(1, (5 * h_posemb * w_posemb), self.dim_tokens))
            nn.init.normal_(self.pos_emb, std=init_std)
