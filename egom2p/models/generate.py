...
from egom2p.utils import get_sentinel_to_id_mapping, merge_span_masking
from egom2p.utils.generation import cosine_schedule, linear_schedule, onex_temp_schedule, linear_temp_schedule, continue_schedule
...
def init_empty_target_modality(mod_dict, modality_info, domain, batch_size, num_tokens, device):
    ...
    """
    if modality_info[domain]['type'] in ['img', 'gaze', 'cam', 'keypoints']:
        # Initialize mod dict
        mod_dict[domain] = {
...
def init_full_input_modality(mod_dict, modality_info, domain, device, eos_id=3):
    ...
    if modality_info[domain]['type'] in ['img', 'cam', 'gaze', 'keypoints']:
...
class GenerationSampler(nn.Module):
    """Sampler that wraps a trained EgoM2P model for generation use cases.
    ...
    def generate(self, mod_dict, schedule, top_k=0.0, top_p=0.0, text_tokenizer=None, verbose=False, seed=None):
        ...
        for step, schedule_step_info in tqdm(enumerate(schedule), disable=not verbose):
            ...
            if self.model.modality_info[target_mod]['type'] in ['img', 'cam', 'gaze', 'keypoints']:
