...
from egom2p.utils.plotting_utils import decode_depth
...
vid_tok_enc = CausalVideoTokenizer(checkpoint_enc=f'./ckpt/encoder.jit')

def encode_mp4_to_token(file_path):
    vr = VideoReader(file_path, ctx=cpu(0))
    vid_len = len(vr)
    video_data = vr.get_batch(range(vid_len)).asnumpy()
    batch_video = video_data[np.newaxis, ...]
    input_tokens = vid_tok_enc(batch_video)
    return input_tokens
...
for file in glob.glob('example_data/rgb2dep_*.mp4'): # make sure all videos are in 8fps, 256x256, 2-second
    ...
    dec_dict = decode_depth(
        name,
        out_dict, toks,
        image_size=256, patch_size=8,
        name = 'example_data/rgb2depth'
    )
